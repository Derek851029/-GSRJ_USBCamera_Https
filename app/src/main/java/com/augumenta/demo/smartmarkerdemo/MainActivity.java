...
public class MainActivity extends AppCompatActivity {
	...
	//    private String[] step = {"https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=1","https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=2","https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=3",
//            "https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=4","https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=5","https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=6",
//            "https://210.68.227.123:8030/mISSION/MissionContext/1?sysid=7"};
	private String Task = "";
			...
			switch (msg.what) {
				...
				case DSpotterStatus.STATUS_RECOGNITION_OK:
					...
					switch (Recog_text){
						...
						case "住民確認":
							if(Bed_SYSID != null){
								if(Now_url.equals("https://"+IP+"/Customer/Login/"+Bed_SYSID)){
									try {
	...
	public void Change_IP_List(){
		url[1] = "https://"+IP+"/Customer/Index";
		customer_list[0] = "https://"+IP+"/Customer/Index/0";
		customer_list[1] = "https://"+IP+"/Customer/Index/1";
		customer_list[2] = "https://"+IP+"/Customer/Index/2";
		customer_list[3] = "https://"+IP+"/Customer/Index/3";
	}
	...
	public void Shared_Data(){
		...
		if(list_step.equals("") == false){
			...
			url[2] = "https://"+ IP + "/Mission/Index/"+Bed_ID+"?missionID="+Bed_MissionID;
		}
	}
		...
		if(bundle != null){
			if(bundle.containsKey("Now_url")){
				...
				if(DispatchSystem.equals("") == false){
					url[0] = "https://"+IP+"/GlassesLogin/Login/"+DispatchSystem;
					if(url_str.equals("https://"+IP+"/GlassesLogin/Login/"+DispatchSystem)){
						init(url[0]);
						Now_url = "https://"+IP+"/GlassesLogin/Login/"+DispatchSystem;
						return;
					}
				}
				...
				if(Bed_SYSID.equals("") == false){
					...
					if(Arrays.asList(customer_list).contains(url_str)){
						init("https://"+IP+"/Customer/Login/"+Bed_SYSID);
						Now_url = "https://"+IP+"/Customer/Login/"+Bed_SYSID;
					}
					else {
						init(url_str);
						Now_url = url_str;
...
//                    + ".bin";
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			if(Bed_MissionID.equals("null") == false){
				...
				HttpGet httpGet = new HttpGet("https://"+IP+"/api/MissionAPI/"+Bed_MissionID);
				try(CloseableHttpResponse response = httpclient.execute(httpGet)) {
					...
					if(res_data.equals("[]")){
						...
						for(int i = 0; i<jsonArray.length(); i++){
							...
							list[i] = "https://"+IP+"/mISSION/MissionContext/" + MissionID +"?sysid="+ SYSID;
							MissionID_array[i] = MissionID;
							SYSID_array[i] = SYSID;

							if(MissionName.equals("傷口拍照")){
								TakePicture_url = "https://"+IP+"/mISSION/MissionContext/" + MissionID +"?sysid="+ SYSID;
							}

							if(MissionName.equals("用藥三讀五對")){
								medicine_url = "https://"+IP+"/mISSION/MissionContext/" + MissionID +"?sysid="+ SYSID;
							}
						}
	...
	Runnable runnable3 = new Runnable() {
		...
		public void run() {
			...
			CloseableHttpClient httpclient = HttpClients.createDefault();
			HttpPost httpPost = new HttpPost("https://"+IP+"/api/MissionAPI/");
...
//                System.out.println(res_data);
	...
	Runnable runnable4 = new Runnable() {
		...
		public void run() {
			CloseableHttpClient httpclient = HttpClients.createDefault();
			HttpPost httpPost = new HttpPost("https://"+IP+"/api/MissionCleanAPI/");
			List<NameValuePair> nvps = new ArrayList<NameValuePair>();
...
//                System.out.println(res_data);
	...
	Runnable runnable5 = new Runnable() {
		...
		public void run() {
			if(Bed_MissionID.equals("null") == false){
				CloseableHttpClient httpclient = HttpClients.createDefault();
				HttpGet httpGet = new HttpGet("https://"+IP+"/api/MedicineAPI/"+Bed_MissionID);
				try(CloseableHttpResponse response = httpclient.execute(httpGet)) {
					System.out.println(response.getCode() + " " + response.getReasonPhrase());
					HttpEntity entity = response.getEntity();
					String res_data  = EntityUtils.toString(entity); //response
					if(res_data.equals("[]")){
						...
						for(int i = 0; i<jsonArray.length(); i++){
							JSONObject jsonObject = jsonArray.getJSONObject(i);
							String SYSID = jsonObject.getString("SYSID");
							medicine_list[i] = "https://"+IP+"/Mission/MedicineList/"+SYSID;
	...
	Runnable runnable6 = new Runnable() {
		...
		public void run() {
			CloseableHttpClient httpclient = HttpClients.createDefault();
			HttpPost httpPost = new HttpPost("https://"+IP+"/api/MedicineAPI/");
			List<NameValuePair> nvps = new ArrayList<NameValuePair>();
			nvps.add(new BasicNameValuePair("MissionID", MissionID_array[0]));
